...
pub use crate::ws_protocol::{MatchMode, Response, Tab};
use crate::{
    routing::parse_url,
...
pub struct Container { pub name: String, #[serde(rename="cookieStoreId")] pub cookie_store_id: String }
...
impl BoundServer {
    ...
    pub fn start(self) -> ServerHandle {
        ...
        tokio::spawn(serve(self.listener, self.token, registry.clone(), receiver, error.clone()));
...
impl ServerHandle {
    pub fn error(&self) -> Option<String> { self.0.error.lock().unwrap_or_else(|error| error.into_inner()).clone() }
    pub fn port(&self) -> u16 {
        self.0.port
    }
    ...
    pub fn instances(&self) -> Vec<Instance> {
        let registry = self.0.registry.lock().unwrap_or_else(|error| error.into_inner());
    ...
    pub fn tabs_digest(&self) -> Vec<InstanceDigest> {
        ...
        for (id, client) in registry.iter() {
            ...
            for tab in &client.tabs {
                let Ok(url) = parse_url(&tab.url) else { continue };
                let Some(host) = url.host_str() else { continue };
                if seen.insert((host.to_owned(), tab.cookie_store_id.clone())) {
                    tabs.push(DigestTab {
                        host: host.to_owned(),
    ...
    pub async fn focus_or_open(
        ...
        self.focus_or_open_with_options(browser, input, mode, None, None).await
    }

    pub async fn focus_or_open_with_options(&self, browser: &str, input: &str, mode: MatchMode, container: Option<&str>, profile: Option<&str>) -> Result<Option<Response>, String> {
        ...
        let (instance_id, receiver) = {
            let mut registry = self.0.registry.lock().unwrap_or_else(|error| error.into_inner());
                ...
                .map(|(id, client)| {
                    (
                        ...
                        client
                            .tabs
                            .iter()
                            .filter(|tab| container.is_none_or(|name| tab.cookie_store_id.as_deref() == Some(name) || client.containers.iter().any(|c| c.name == name && tab.cookie_store_id.as_deref() == Some(c.cookie_store_id.as_str()))))
            ...
            let message = Message::Text(
                json!({"id":id,"action":"FOCUS_OR_OPEN","url":url.as_str(),"match_mode":mode,"container":container,"profile":profile,"deadline_ms":request_deadline_ms()})
                    .to_string(),
            );
            client
                .tx
        ...
        match timeout(REQUEST_TIMEOUT, receiver).await {
            ...
            Ok(Ok(response)) if response.result == "ERROR_CONTAINER_NOT_FOUND" => Ok(Some(response)),
            ...
            Ok(Ok(response)) if browser == "edge" && response.result == "ERROR_NO_BROWSER_WINDOW" => Ok(Some(response)),
    ...
    /// already be closed.
    pub async fn close_tabs_with_options(&self, browser: &str, input: &str, mode: MatchMode, container: Option<&str>, profile: Option<&str>) -> Result<Option<Response>, String> {
        let url = parse_url(input)?;
        if url.as_str().len() > MAX_URL_BYTES {
            return Err("Companion URL exceeds 2048 bytes".into());
        }
        ...
        match timeout(REQUEST_TIMEOUT, receiver).await {
            ...
            Ok(Err(_)) => {
                Err("Companion disconnected; the tabs may already be closed".into())
            }
...
async fn connection(stream: TcpStream, token: &str, registry: Registry) {
    ...
    if !matches!(
        timeout(
            AUTH_TIMEOUT,
            socket.send(Message::Text(json!({"type":"AUTH_OK","capabilities":["paged_tabs_v1"]}).to_string()))
        )
        .await,
        Ok(Ok(()))
    ) {
...
fn receive(value: Value, registry: &Registry, instance_id: &str, browser: &str) -> bool {
    ...
    if value.get("action").and_then(Value::as_str) == Some("CONTAINERS_LIST") {
        let Some(value) = value.get("containers") else { return false; };
        let Ok(containers) = serde_json::from_value::<Vec<Container>>(value.clone()) else { return false; };
        if containers.len() > 200 || containers.iter().any(|c| c.name.len() > 128 || !crate::options::valid_name(&c.cookie_store_id)) { return false; }
        client.containers = containers;
    } else if value.get("action").and_then(Value::as_str) == Some("TABS_SYNC_PAGE") {
        let Ok(page) = serde_json::from_value::<TabPage>(value) else { return false; };
        if !page.sync.valid(browser) || page.pages == 0 || page.pages > 10 || page.page >= page.pages
        ...
        if page.page == 0 {
            client.snapshot = Some(TabSnapshot {
                id: page.snapshot_id.clone(), pages: page.pages, next: 0,
                started: now, tabs: Vec::new(),
            });
        }
        let Some(snapshot) = client.snapshot.as_mut() else { return false; };
        if snapshot.id != page.snapshot_id || snapshot.pages != page.pages || snapshot.next != page.page
            || now.duration_since(snapshot.started) >= Duration::from_secs(5) {
            return false;
        }
        let ids: HashSet<_> = snapshot.tabs.iter().map(|t| t.id).collect();
        if page.sync.tabs.iter().any(|t| ids.contains(&t.id)) { return false; }
        ...
        if snapshot.next == snapshot.pages {
            ...
            let Some(snapshot) = client.snapshot.take() else { return false; };
