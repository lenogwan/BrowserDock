...
use browserdock_launcher::{config::Browser, groups::{self, Group}, vault::Bookmark};
...
enum Mutation {
    Save(Group),
    Delete(String),
    Move { id: String, group_id: Option<String>, index: u32 },
}

async fn mutate(app: tauri::AppHandle, private: bool, operation: Mutation) -> Result<(), String> {
    let epoch = app.try_state::<DesktopState>().ok_or("Configuration unavailable")?.gate.ticket();
    tauri::async_runtime::spawn_blocking(move || {
        ...
        if private {
            ...
            let result = match operation {
                ...
                Mutation::Move { id, group_id, index } => vault.move_bookmark(&id, group_id, index, now),
            };
            notify_lock(&app, &state, &mut vault);
            return result;
        }
        let mut config = state.config.lock().map_err(|_| "Configuration unavailable")?;
        ...
        let mut bookmarks: Vec<Bookmark> = next.bookmarks.iter()
        ...
        match operation {
            ...
            Mutation::Move { id, group_id, index } => {
                groups::move_bookmark(&mut bookmarks, &next.groups, &id, group_id, index)?;
            },
        }
        for bookmark in &bookmarks {
            if let Some(value) = next.bookmarks.iter_mut().find(|v|
                v.get("id").and_then(|v| v.as_str()) == Some(bookmark.id.as_str())) {
        ...
        *config = next;
        Ok(())
    }).await.map_err(|_| "Organization task failed")?
}
...
#[tauri::command]
pub async fn move_bookmark(app: tauri::AppHandle, id: String, group_id: Option<String>, index: u32, private: bool) -> Result<(), String> {
    mutate(app, private, Mutation::Move { id, group_id, index }).await
}
...
pub async fn save_browser(app: tauri::AppHandle, browser: Browser) -> Result<(), String> {
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let mut config = state.config.lock().map_err(|_| "Configuration unavailable")?;
        let mut next = config.clone();
        let existing = next.browsers.iter_mut().find(|b| b.id == browser.id)
        ...
        *config = next;
        Ok(())
    }).await.map_err(|_| "Browser settings task failed")?
}
