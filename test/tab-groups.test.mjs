...
import { buildBrowserGroupIndex, browserGroupFor, browserGroupFromIndex, hasBrowserGroup, hasBrowserGroupInIndex, groupColor } from '../src/lib/features/bookmarks/tab-groups.js';
...
test('native group badges resolve containers and browser defaults', () => {
  const index=buildBrowserGroupIndex(instances,[{id:'firefox',container:'Work'}]);
  assert.equal(browserGroupFromIndex(bookmark,index).groupTitle,'Research');
  assert.equal(browserGroupFromIndex({...bookmark,browser_options:{}},index).groupColor,'blue');
...
test('close group availability follows native title in the routed container', () => {
  const index=buildBrowserGroupIndex(instances,[]);
  assert.equal(hasBrowserGroupInIndex({name:'Research'},[bookmark],index),true);
  assert.equal(hasBrowserGroupInIndex({name:'Research'},[{...bookmark,browser_options:{incognito:true}}],index),false);
